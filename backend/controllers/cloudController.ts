...
import {
  getCloudProjectDetails,
  getCloudProjectsByCategory,
} from "../services/cloudService.js";

export const listCloudProjectsController = async (
  _req: Request,
  res: Response,
) => {
  try {
...
export const getCloudProjectDetailsController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const hubId = decodeURIComponent(
      Array.isArray(hubParam) ? hubParam[0] || "" : hubParam || "",
    ).trim();
